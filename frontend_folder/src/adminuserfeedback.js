...
import './footer.css'
import { Table,Container,Row,Dropdown,Modal,Button } from 'react-bootstrap';
...
const Adminuserfeedback = () => {
  ...
  const handleShow = (e,item,item1) =>{
    e.preventDefault()
    setShow(true)
    setSavefeedback(item)
    setUser(item1)

  } 
  const handleClose1 = () => setShow1(false);
  const handleShow1 = (e,email,id) =>{
    setShow1(true)
    setUseremail(email)
    setUserid(id)
      ...
      mailfeedback:"" });
        ...
        const handleClick = async (e) => {
         ...
         const emails=useremail
          const { mailfeedback } = usering;
          const res = await fetch("http://localhost:5000/api/feedbackmail", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
...
<span style={{fontWeight:'bolder',fontSize:'30px',textDecoration:'underline',textDecorationColor:'#EEB127',textAlign:'center',textDecorationThickness:'8px',fontFamily:'sans-serif',marginTop:'1%'}}>USER FEEDBACK</span>
</Row></Container>
<br/>
...
<Modal show={show} onHide={handleClose}>
        ...
        <Table striped bordered hover>
            <thead>
              ...
              <tbody>
          {savefeedback.length>0 ? savefeedback.map((item,index)=>{
          return(
                <tr key={index}>
                  <td>{++index}</td>
                  <td>{item}</td>
        ...
        }):<tr><h2 style={{textAlign:'center'}}>No feedback yet</h2></tr>
      } </tbody>
        </Table></Modal.Body>
        <Modal.Footer>
      ...
      <Modal show={show1} onHide={handleClose1}>
        <Modal.Header closeButton>
          <Modal.Title>Write mail to {useremail}</Modal.Title>
        </Modal.Header>
        <Modal.Body><form method="POST">
        <label htmlFor="mailfeedback">Write mail...</label>
          <textarea id="mailfeedback" name="mailfeedback" defaultValue={usering.mailfeedback} onChange={handleInputs} required style={{width: '100%',padding: '12px',marginTop: '6px',  marginBottom: '16px',resize: 'vertical' }} ></textarea>
          <button style={{color:'white',backgroundColor:'#05386B'}} onClick={handleClick}>Send mail</button>
          </form></Modal.Body>
        <Modal.Footer>
